...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            self.velocities[k] = momentum*velocities[k] + learning_rate*grads[k]
            params[k] = params[k] - self.velocities[k]
...
class AdaGradOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            # get the sum of squares of all the gradients from the previous timestep for given param
            sum_squared_gradients = cache[k] + grads[k]**2
            
            param_specific_learning_rate = learning_rate/np.sqrt(sum_squared_gradients + eps)
            params[k] = params[k] - param_specific_learning_rate*grads[k]

            # update the cache of previous gradients with the current gradients
            self.cache[k] = sum_squared_gradients
        ###################################################
        #               END OF YOUR CODE                  #
        ###################################################
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            # set moment 1 and moment 2
            m_t = beta1*self.momentums[k] + (1- beta1)*grads[k]
            v_t = beta2*self.velocities[k] + (1- beta2)*grads[k]**2
            ...
            params[k] = params[k] - learning_rate*m_t/(np.sqrt(v_t) + eps)

        self.t = t
        self.momentums[k] = m_t
        self.velocities[k] = v_t
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            nadam_momentum = beta1*m_t + ((1-beta1)*grads[k])/(1-beta1**t)

            params[k] = params[k] - learning_rate*nadam_momentum/(np.sqrt(v_t) + eps)
