...
def dropout_forward(x, dropout_config, mode):
    ...
    if mode == "train":
        ...
        # multiply X by a matrix of 0s and 1s with 0s 
        # appearing keep_probability percent of the time in the matrix
        
        dropout_mask = np.random.choice([0, 1], size=x.shape, p=[1-keep_prob, keep_prob])
        out = dropout_mask*x
        cache = dropout_mask
        ...
        # multiply x by keep_prob to simulate averaging over
        # the different "networks" created with the different
        # dropout masks for iteration
