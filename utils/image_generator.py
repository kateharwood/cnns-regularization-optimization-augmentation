...
class ImageGenerator(object):
    ...
    def create_aug_data(self):
        ...
        if self.noisy:
            self.x_aug = np.vstack((self.x_aug,self.noisy[0]))
            self.y_aug = np.hstack((self.y_aug,self.noisy[1]))
        if self.bright:
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        if batch_size > self.N:
            print("Batch size requested is larger than number of available training examples.")
        else:
            while True:
                if batch_count < np.floor(self.N/batch_size):
                    batch_count = batch_count + 1

                    # yield batch_size examples, starting from the end of the previous batches yielded
                    next_batch_x = self.x_aug[starting_point:batch_count*batch_size]
                    next_batch_y = self.y_aug[starting_point:batch_count*batch_size]
                    starting_point = batch_count*batch_size
                    yield next_batch_x, next_batch_y
                else:
                    index = np.random.permutation(len(self.x_aug))
                    self.x_aug = self.x_aug[index]
                    self.y_aug = self.y_aug[index]
                    batch_count = 0
                    starting_point = 0
    ...
    def show(self, images):
        ...
        for i in range(16):
            ...
            ax.imshow(images[i, :].reshape(28, 28), 'gray') #TODO should this be x or x_aug?
            ax.axis('off')
    ...
    def translate(self, shift_height, shift_width):
        ...
        translated = np.roll(self.x, (shift_height, shift_width), axis=(1,2))
        
        self.N_aug += self.N
        self.translated = (translated,self.y.copy())
        self.num_pixels_translated = (shift_height, shift_width)
        return translated
    ...
    def rotate(self, angle=0.0):
        ...
        rotated = rotate(self.x, angle, axes=(1,2))
        
        self.N_aug += self.N
        self.rotated = (rotated,self.y.copy())
        self.degree_rotated = angle
        return rotated
    ...
    def add_noise(self, portion, amplitude):
        ...
        :return noisy: dataset with noise added
        """
        ...
        noise = np.random.uniform(-amplitude, amplitude, self.x.shape)
        mask = np.random.binomial(n=1, p=portion, size=self.x.shape)
        noisy = self.x + mask*noise
        
        self.N_aug += self.N
        self.noisy = (noisy,self.y.copy())
