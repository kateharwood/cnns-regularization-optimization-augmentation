...
def conv2d_forward(x, w, b, pad, stride):
    ...
    # pad the width and height of the image          
    x = np.pad(x, ((0,0), (1,1), (1,1), (0,0)), mode='constant')
    
    num_filters = b.shape[0]
    batch = x.shape[0]

    # set up output matrix
    output_height = ((x.shape[1] - w.shape[0]) // stride) + 1
    output_width = ((x.shape[2] - w.shape[1]) // stride) + 1
    output = np.zeros(shape=(batch, output_width, output_height, num_filters))
        ...
        for m in range(output_height):
            for n in range(output_width):
                for f in range(num_filters):
                    current_image = x[image]
                    current_output_image = output[image]
                    current_filter = w[:,:,:,f]
                    current_convolve_location = current_image[m*stride:m*stride+filter_height,n*stride:n*stride+filter_width,:]
                    current_output_image[m,n,f] = np.sum(current_convolve_location * current_filter) + b[f] 
        output[image] = current_output_image
    return(output)        
...
def avg_pool_forward(x, pool_size, stride):
    ...
    output = np.zeros(shape=(batch,output_height, output_width, depth))
    
    for feat_map in range(batch):
        curr_feat_map = x[feat_map]
        for m in range(output_height):
            for n in range(output_width):
                for d in range(depth):
                    current_window = curr_feat_map[m*stride:m*stride+pool_size,n*stride:n*stride+pool_size,d]
                    average = np.average(current_window)
                    curr_output_feat_map = output[feat_map]
                    curr_output_feat_map[m,n,d] = average 
        output[feat_map] = curr_output_feat_map
